...
from .configuration import ARQMATH_COLLECTION_INPUT_DATA_DIRNAME, ARQMATH_COLLECTION_POSTS_SLT_FILENAME, POOL_NUM_WORKERS, POOL_CHUNKSIZE, ARQMATH_TRAIN_TSV_SLT_FILENAME, ARQMATH_TRAIN_TSV_SLT_NUM_FORMULAE, CSV_PARAMETERS, ARQMATH_TASK1_TEST_TSV_SLT_FILENAME, ARQMATH_TASK1_TEST_TSV_SLT_NUM_FORMULAE, ARQMATH_TASK1_TEST_INPUT_POSTS_FILENAME, ARQMATH_TASK1_TEST_POSTS_SLT_FILENAME
...
if sys.argv[1] == 'collection':
    ...
    ARQMATH_TSV_SLT_NUM_FORMULAE = ARQMATH_TRAIN_TSV_SLT_NUM_FORMULAE
    ARQMATH_POSTS_SLT_FILENAME = ARQMATH_COLLECTION_POSTS_SLT_FILENAME
elif sys.argv[1] == 'task1-topics':
    ARQMATH_TSV_SLT_FILENAME = ARQMATH_TASK1_TEST_TSV_SLT_FILENAME
    ARQMATH_TSV_SLT_NUM_FORMULAE = ARQMATH_TASK1_TEST_TSV_SLT_NUM_FORMULAE
    ARQMATH_POSTS_SLT_FILENAME = ARQMATH_TASK1_TEST_POSTS_SLT_FILENAME


def count_tsv():
    with open(ARQMATH_TSV_SLT_FILENAME, 'rt') as f:
        rows = csv.reader(f, **CSV_PARAMETERS)
        num_rows = sum(1 for _ in tqdm(rows, desc='Counting formulae')) - 1
    assert num_rows == ARQMATH_TSV_SLT_NUM_FORMULAE, '{} contains {} formulae instead of the expected {}'.format(
        ARQMATH_TSV_SLT_FILENAME,
        num_rows,
        ARQMATH_TSV_SLT_NUM_FORMULAE,
    )
    return num_rows
...
if __name__ == '__main__':
    with gzip.open(ARQMATH_POSTS_SLT_FILENAME, 'wt') as f:
        print('{', file=f)
        if sys.argv[1] == 'collection':
