...
from .configuration import ARQMATH_COLLECTION_INPUT_DATA_DIRNAME, ARQMATH_COLLECTION_POSTS_INFIX_FILENAME, POOL_NUM_WORKERS, POOL_CHUNKSIZE, ARQMATH_TRAIN_TSV_INFIX_FILENAME, ARQMATH_TRAIN_TSV_INFIX_NUM_FORMULAE, CSV_PARAMETERS
from .common import arqmath_post_read_html5_worker as read_html5_worker, Text, Math


def count_tsv():
    with open(ARQMATH_TRAIN_TSV_INFIX_FILENAME, 'rt') as f:
        rows = csv.reader(f, **CSV_PARAMETERS)
        num_rows = sum(1 for _ in tqdm(rows, desc='Counting formulae')) - 1
    assert num_rows == ARQMATH_TRAIN_TSV_INFIX_NUM_FORMULAE, '{} contains {} formulae instead of the expected {}'.format(
        ARQMATH_TRAIN_TSV_INFIX_FILENAME,
        num_rows,
        ARQMATH_TRAIN_TSV_INFIX_NUM_FORMULAE,
    )
    return num_rows
...
if __name__ == '__main__':
    with gzip.open(ARQMATH_COLLECTION_POSTS_INFIX_FILENAME, 'wt') as f:
        print('{', file=f)
