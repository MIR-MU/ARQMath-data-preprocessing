...
from .common import ntcir_article_read_xhtml_worker as read_xhtml_worker, Math, opt_tokenize as tokenize
from .configuration import POOL_NUM_WORKERS, POOL_CHUNKSIZE, NTCIR12_MATHIR_MATHWIKIFORMULA_DATA_XHTML_GLOB, NTCIR12_MATHIR_MATHWIKIFORMULA_DATA_JSON_OPT_FILENAME, NTCIR12_MATHIR_MATHWIKIFORMULA_DATA_JSON_OPT_FAILURES_FILENAME, NTCIR12_MATHIR_MATHWIKIFORMULA_DATA_XHTML_NUM_DOCUMENTS, XML_NAMESPACES
...
def write_json():
    ...
    num_successful = 0
    num_total = 0
    with gzip.open(NTCIR12_MATHIR_MATHWIKIFORMULA_DATA_JSON_OPT_FILENAME, 'wt') as f, open(NTCIR12_MATHIR_MATHWIKIFORMULA_DATA_JSON_OPT_FAILURES_FILENAME, 'wt') as failures_f:
        print('{', file=f)
        with Pool(POOL_NUM_WORKERS) as pool:
